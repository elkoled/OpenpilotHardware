...
struct PinMapping {
  int mcp_index;    // Which MCP chip (0-3 for MCP1-4)
  char port;        // 'A' or 'B'
  int pin;          // Pin number (0-7)
  bool valid;       // Is this pin connected?
};

// Lookup table: Pin -> MCP
...
// helper functions
void digitalWritePin(int pin, int value, bool is_plug = false) {
  uint8_t offset = 0;
  ...
  if (mapping->port == 'A') {
    mcp[mapping->mcp_index + offset].digital_write_a(mapping->pin, value);
  } else {
    mcp[mapping->mcp_index + offset].digital_write_b(mapping->pin, value);
  }
}

int digitalReadPin(int pin, bool is_plug = false) {
  uint8_t offset = 0;
  PinMapping* mapping = getPinMapping(pin);
  ...
  if (mapping->port == 'A') {
    return mcp[mapping->mcp_index + offset].digital_read_a(mapping->pin);
  } else {
    return mcp[mapping->mcp_index + offset].digital_read_b(mapping->pin);
  }
}

void pinModePin(int pin, int mode, bool is_plug = false) {
  uint8_t offset = 0;
  PinMapping* mapping = getPinMapping(pin);
  if (!mapping) return;
  if(is_plug) offset=4;
  ...
  if (mapping->port == 'A') {
    mcp[mapping->mcp_index + offset].pin_mode_a(mapping->pin, mode);
  } else {
    mcp[mapping->mcp_index + offset].pin_mode_b(mapping->pin, mode);
  }
}
...
void setup() {
  ...
  // Initialize MCP chips as in original code
  for (int i = 4; i < 9; i++) {
    mcp[i].begin();
